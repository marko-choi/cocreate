...
const Canvas = (props) => {
  ...
  const drawSelection = (ctx, x, y, width2, height2, fillStyle = "rgba(200, 200, 200, 0.3)", strokeStyle = "white", lineWidth = 2, radius = 15) => {
    ...
    ctx.arcTo(x + width2, y, x + width2, y + height2, radius);
    ctx.arcTo(x + width2, y + height2, x, y + height2, radius);
    ctx.arcTo(x, y + height2, x, y, radius);
    ctx.arcTo(x, y, x + width2, y, radius);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  };
