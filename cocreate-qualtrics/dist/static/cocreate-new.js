...
function deepClone(source) {
  if (/* @__PURE__ */ reactExports.isValidElement(source) || reactIsExports$1.isValidElementType(source) || !isPlainObject(source)) {
    return source;
  }
  const output = {};
...
const Tooltip = (props) => {
  const { index, x: x2, y: y2, selection, setSelections, setTooltipPosition, setIsEnteringFeedback, setActiveSelectionIndex, onDelete, annotation } = props;
  ...
  const saveChanges = () => {
    setSelections((prev2) => {
      ...
      newSelections[index] = {
        ...
        comment: feedbackConfig.showComment ? comment2 ?? "" : ""
      };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      ...
      children: [
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(Divider$1, { style: { marginBottom: "8px" } }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: "8px" }, children: [
          feedbackConfig.showFunctionValue && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }, children: [
          ...
          feedbackConfig.showAestheticValue && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { style: { marginBottom: "8px", display: "flex", justifyContent: "space-between", alignItems: "center" }, children: [
          ...
          feedbackConfig.showComment && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("div", { children: "Additional Comments" }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "textarea",
              {
                value: comment2,
                onChange: (e2) => handleComment(e2.target.value),
                rows: 3,
                disabled: annotation,
                style: {
                  width: "100%",
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsx(Divider$1, { style: { marginBottom: "12px" } }),
...
const isMobileDevice = () => {
  ...
  return isMobileUA && hasTouchScreen || hasTouchScreen && isMobileWidth && hasOrientationAPI;
};
const MobileFeedbackModal$1 = "";
const MobileFeedbackModal = ({
  visible,
  selection,
  onSave,
  onDelete,
...
const Canvas = (props) => {
  ...
  reactExports.useEffect(() => {
    ...
    const ctx = canvas2 == null ? void 0 : canvas2.getContext("2d");
    if (canvas2 && ctx) {
      ctx.clearRect(0, 0, canvas2.width, canvas2.height);
      redrawSelections(ctx);
    }
  }, [selections]);
  const drawSelection = (ctx, x2, y2, width2, height2, fillStyle = "rgba(200, 200, 200, 0.3)", strokeStyle = "white", lineWidth = 2, radius = 15) => {
    ctx.fillStyle = fillStyle;
    ctx.strokeStyle = strokeStyle;
    ctx.lineWidth = lineWidth;
    ctx.beginPath();
    ctx.moveTo(x2 + radius, y2);
    ctx.arcTo(x2 + width2, y2, x2 + width2, y2 + height2, radius);
    ctx.arcTo(x2 + width2, y2 + height2, x2, y2 + height2, radius);
    ctx.arcTo(x2, y2 + height2, x2, y2, radius);
    ctx.arcTo(x2, y2, x2 + width2, y2, radius);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
  };
  const redrawSelections = (ctx, resizedSelections) => {
    ...
    selectionsToDraw.forEach((selection, index) => {
      if (isCircularSelection(selection)) {
        ...
        drawSelection(ctx, x2, y2, width2, height2);
      }
    });
  };
  const toStagePointFromEvent = (e2, element) => {
    const rect = element.getBoundingClientRect();
    return {
      x: (e2.clientX - rect.left) / scale,
      y: (e2.clientY - rect.top) / scale
    };
  };
  const stageToScreenPoint = (p2) => {
    return { x: translate.x + scale * p2.x, y: translate.y + scale * p2.y };
  };
  ...
  const handleMouseDown = (e2) => {
    ...
    if (isPanMode) {
      setIsPanning(true);
      panLastRef.current = { x: e2.clientX, y: e2.clientY };
      return;
    }
    removeEmptyFeedback();
    ...
    setActiveSelectionIndex(null);
    registerSelection(e2, canvas2);
  };
  const registerSelection = (e2, canvas2) => {
    const stagePoint = toStagePointFromEvent(e2, canvas2);
  ...
  const handleMouseMove = (e2) => {
    ...
    if (isPanMode && isPanning && panLastRef.current) {
      const dx = e2.clientX - panLastRef.current.x;
      const dy = e2.clientY - panLastRef.current.y;
      setTranslate((prev2) => constrainTranslate({ x: prev2.x + dx, y: prev2.y + dy }, scale));
      panLastRef.current = { x: e2.clientX, y: e2.clientY };
      return;
    }
    if (!isSelecting || !selectionStart || isEnteringFeedback)
      return;
    ...
    ctx.clearRect(0, 0, canvas2.width, canvas2.height);
    redrawSelections(ctx);
    const x2 = Math.min(selectionStart.x, currentEnd.x);
    const y2 = Math.min(selectionStart.y, currentEnd.y);
    const width2 = Math.abs(currentEnd.x - selectionStart.x);
    const height2 = Math.abs(currentEnd.y - selectionStart.y);
    drawSelection(ctx, x2, y2, width2, height2);
    checkIfMouseIsInsideCanvas(e2);
  };
  const handleMouseLeave = (e2) => {
    ...
    const clampedX = Math.max(0, Math.min(mouseX, canvas2.width));
    const clampedY = Math.max(0, Math.min(mouseY, canvas2.height));
  ...
  const checkIfMouseIsInsideCanvas = (e2) => {
    const canvas2 = canvasRef.current;
    if (!canvas2)
      return;
    const rect = canvas2.getBoundingClientRect();
  ...
  const handleMouseUp = (e2) => {
    ...
    if (sameXCoordinate && sameYCoordinate) {
      if (!isEnteringFeedback && allowPictureSelection) {
        ...
        if (!canCreatePictureSelection(width2, height2)) {
          openPictureSelectionFeedback(e2);
    ...
    const x2 = Math.min(selectionStart.x, selectionEnd.x);
    const y2 = Math.max(selectionStart.y, selectionEnd.y) + 8;
  ...
  const openPictureSelectionFeedback = (e2) => {
    ...
    if (pictureSelection) {
      ...
      const mouseCoordinates2 = { x: e2.clientX, y: e2.clientY };
    ...
    createPictureSelection(width2, height2);
    const newIndex = selections.length;
    const mouseCoordinates = { x: e2.clientX, y: e2.clientY };
    setActiveSelectionIndex(newIndex);
  ...
  const canCreatePictureSelection = (width2, height2) => {
    ...
    if (pictureWideSelection.length !== 0)
      return false;
    return true;
  };
  ...
  const handleEdit = (index) => {
    ...
    const { x: x2, y: y2, width: width2, height: height2 } = selectionBoundsInStage(selection);
    console.debug("Tooltip Position: ", { x: x2, y: y2 + height2 + 8 }, x2, y2, width2, height2);
    setTooltipPosition({ x: x2, y: y2 + height2 + 8 });
    setTooltipAnchoredToSelection(true);
  ...
  const checkForPictureSelection = (index) => {
    ...
    if (!canvas2)
      return;
    const { width: width2, height: height2 } = canvas2.getBoundingClientRect();
  ...
  const constrainTranslate = (newTranslate, newScale) => {
    ...
    let x2 = newTranslate.x;
    let y2 = newTranslate.y;
    if (scaledWidth <= containerWidth) {
      x2 = (containerWidth - scaledWidth) / 2;
    } else {
    ...
    if (scaledHeight <= containerHeight) {
      y2 = (containerHeight - scaledHeight) / 2;
    } else {
  ...
  const handleWheelNative = (e2) => {
    ...
    const cursorStageX = (e2.clientX - rect.left) / scale;
    const cursorStageY = (e2.clientY - rect.top) / scale;
    setTranslate((prev2) => {
  ...
  const minimap = reactExports.useMemo(() => {
    ...
    const k2 = miniWidth / canvasWidth;
    return {
      width: miniWidth,
      height: miniHeight,
      rect: {
        x: Math.max(0, Math.min(viewX * k2, miniWidth)),
        y: Math.max(0, Math.min(viewY * k2, miniHeight)),
        w: Math.max(0, Math.min(viewWidth * k2, miniWidth)),
        h: Math.max(0, Math.min(viewHeight * k2, miniHeight))
      }
    };
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "canvas-shell", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs(
      "div",
      {
        ...
        children: [
          !isMobile && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "zoom-toolbar", style: { display: toolbarVisible ? "flex" : "none" }, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              IconButton$1,
              {
                size: "small",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              ...
              {
                size: "small",
                onClick: () => setMinimapVisible((v2) => !v2),
                onMouseEnter: (e2) => {
                  const rect = e2.currentTarget.getBoundingClientRect();
          ...
          !isMobile && selections.map((selection, index) => {
            const { x: x2, y: y2, width: width2, height: height2 } = selectionBoundsInStage(selection);
            const screenTopLeft = stageToScreenPoint({ x: x2, y: y2 });
    ...
    !isMobile && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "canvas-top-right-controls", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
      IconButton$1,
      {
        size: "small",
        onClick: () => {
          setToolbarVisible((v2) => !v2);
          if (toolbarVisible)
            setMinimapVisible(false);
        },
